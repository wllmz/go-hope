...
const UpdateSousCategoryModal = ({
  ...
  useEffect(() => {
    if (sousCategory) {
      setFormData({
        nameSousCategorie: sousCategory.nameSousCategorie,
        ageCible: sousCategory.ageCible,
      });
    }
  }, [sousCategory]);
  ...
  const handleConfirm = () => {
    ...
    onConfirm(sousCategory._id, formData);
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <h2 className="text-lg font-bold mb-4">Modifier la Sous-Catégorie</h2>
      <input
        type="text"
...
export default UpdateSousCategoryModal;
